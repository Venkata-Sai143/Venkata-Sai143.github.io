...
function loadExperienceDetails() {
    ...
    fetch(
        url +
        '/rest/v1/experiences?select=job_title,company,responsibilities,tools&is_published=eq.true&order=sort_order.asc',
        {
            headers: {
                apikey: key,
                Authorization: 'Bearer ' + key
    ...
    .then(function (response) {
        if (!response.ok) {
            throw new Error('Experience details failed: ' + response.status);
        }
        return response.json();
    })
    .then(function (rows) {
        rows.forEach(function (row) {
            ...
            for (var level = 0; level < 8 && parent; level++) {
                var text = (parent.textContent || '').toLowerCase();

                if (
                    text.indexOf((row.company || '').toLowerCase()) !== -1 &&
                    (
                        text.indexOf((row.description || '').slice(0, 30).toLowerCase()) !== -1 ||
                        level >= 3
                    )
                ) {
                    break;
            ...
            var old = parent.querySelector(
                '[data-backend-experience-details="' + wanted + '"]'
            );
